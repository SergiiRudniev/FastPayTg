...
class CodeManager:
    ...
    def __SetCode(self, id, code):
        try:
            self.redis_client.redis_client.set(f"code_{id}", code, ex=60)
            self.redis_client.redis_client.set(f"attempts_{id}", 0, ex=60)
            return {"status": "ok"}
        except Exception as e:
            return {"status": "error", "error": e}

    def __AddAttempts(self, id):
        self.redis_client.redis_client.set(f"attempts_{id}", self.redis_client.get(f"attempts_{id}")+1)
